...
public class ReactiveForwarding {
    ...
    private class ReactivePacketProcessor implements PacketProcessor {
        ...
        public void process(PacketContext context) {
            ...
            log.info("Packet received from " + src.location().deviceId() + " dsteination " + dst.location().deviceId() );
            if (paths.isEmpty()) {
            ...
            setFLows(path.links(), src.mac(), dst.mac());
...
//        for (Path path : paths) {
//            log.info("### Paths Cost:  " + path.cost());
//            for (Link link : path.links()) {
//                log.info("### Link        src:" + link.src() + "  dst: " + link.dst()+ "  state: " + link.state());
//            }
//        }
        for (Path path : paths) {
            if (!path.src().port().equals(notToPort)) {
                for(Link link : path.links()){
//                    log.info("### Link        src:" + link.src() + "  dst: " + link.dst()+ "  state: " + link.state());
//                    log.info(link.dst().deviceId().toString());
//
//                    log.info("000000000000000"+ (turn+1));
                    if(link.dst().deviceId().toString().equals("of:000000000000000"+ (turn+1))){
                        selected = true;
                    }
                }
                if(selected){
                    turn++;
                    turn = turn % numberOfSpines;
//                    log.info("### selected Paths Cost:  " + path.cost());
//                    for (Link link : path.links()) {
//                        log.info("### Link        src:" + link.src() + "  dst: " + link.dst()+ "  state: " + link.state());
//                    }
                    return path;
    ...
    public void setFlow(DeviceId switchDevice, MacAddress srcMac, MacAddress dstMac, PortNumber outPort){
        setFlow(switchDevice, srcMac, dstMac, outPort,DEFAULT_RULE_TIMEOUT,DEFAULT_RULE_PRIO);
    }
    /**
