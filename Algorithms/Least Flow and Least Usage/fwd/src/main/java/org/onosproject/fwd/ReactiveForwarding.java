...
public class ReactiveForwarding {
    ...
    private final int DEFAULT_RULE_TIMEOUT = 120;
    private final int LOCAL_RULE_TIMEOUT = 600;
    private int LINK_CAPACITY = 20000000;

    private final int test_scenario = 1;
	 ...
	 * It selects path with minimum number of flows on
     *
     * @param hostA Host A-Source
     * @param hostB Host B-Destination
	 * @author Furkan Okuyucu
     */
    public void deployShortestPath(Host hostA, Host hostB) {

        if (flowRuleExists(hostA, hostB)) {
            log.info("--------LEASt FLOW PATH FINDER: Flow already exists! Not deploying Flow-Rules for Host-combo (" + hostA.mac().toString() + ":" + hostB.mac().toString() + ") again! ");
            return;
        }

        log.info("--------LEAST FLOW PATH FINDER Evaluating paths for Host-combo (" + hostA.mac().toString() + ":" + hostB.mac().toString() + ")");
        ...
        //Remove paths whoose hopcount greater than 2
    ...
    public Path bestPathInSet(Set<Path> paths) {
        ...
        if(test_scenario == 1){
            ...
            } else // UsagePercent differs
            {
                ...
                log.info("Path decision based on Utilization Rate" + maxLinkUsage[index] + ")");
                bestPath = pathList[index];
                return bestPath;
            }
        }
        else if(test_scenario == 2){
            log.info("Path decision based on LEAST FLOW COUNT");
            ...
            } else // minFree differs
            {
                ...
                bestPath = pathList[index];
                log.info(" PATH Decision based on Flow-Count: Choosing the path with least flows(" + numberOfFlows[index] + "). Path: "+bestPath.toString());
                return bestPath;
            }
        }
        return bestPath;
